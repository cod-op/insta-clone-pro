...
const SignUp = () => {
 ...
 const handleSignUp=async()=>{
     ...
     setErr(false)
   try{
     const result=await axios.post(backendUrl+'/api/auth/signup',{
  ...
  return (
    <div className="w-full h-screen bg-gradient-to-b from-black to-gray-900  flex flex-col justify-center items-center">
      <div className="w-[90%] lg:max-w-[60%] h-[600px] bg-white rounded-2xl flex justify-center items-center overflow-hidden border-2 border-[#1a1f23]">
        ...
        <div className="w-full lg:w-[50%] h-full bg-white flex flex-col items-center p-[10] gap-[20px]">
            <div className="flex gap-[10px] items-center text-[20px] font-semibold mt-[40px]" >
               <span>Sign Up to</span>
               <img src={logo} alt="" className="w-[90px]" />
            </div>
            <div onClick={()=>setInputClicked({...inputClicked,name:true})} className="relative flex items-center justify-start w-[90%] h-[50px] rounded-2xl  border-2 border-black">
            ...
            <div onClick={()=>setInputClicked({...inputClicked,email:true})} className="relative flex items-center justify-start w-[90%] h-[50px] rounded-2xl border-2 border-black">
           ...
           <p className="text-red-500">{err}</p>}
            ...
            <button onClick={handleSignUp} disabled={loading} className="w-[70%] px-[20px] bg-black text-white font-semibold h-[50px] cursor-pointer rounded-2xl mt-[30px]">
                 {loading?<ClipLoader size={30} color='green'/>:"Sign Up"}    
            </button>
            <p onClick={()=>navigate('/signin')} className="cursor-pointer text-gray-800">Already Have an account ? <span className="border-b-2 border-b-black">Sign In</span></p>
        </div>
